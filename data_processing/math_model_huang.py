...
def calc_cir(config, c_factor, c_alpha, c_velo):
...
def calc_theoretic_cir(config, factor_1, factor_2, alpha_1, alpha_2, velo_1, velo_2, beta_1 = 0, beta_2 = 0):
    config_1 = config.copy_config()
    config_2 = config.copy_config()
    t_cir_1 = calc_cir(config_1, factor_1, alpha_1, velo_1)
    t_cir_2 = calc_cir(config_2, factor_2, alpha_2, velo_2)
    t_cir = []
...
velo = [[20,20,20,20], [1.0,1.0,1.0,1.0]]
...
for c in range(len(configs)):
    ...
    write_results('./data/_huang_', config, timex, cir_data[c,:], cir_theo_data[c,:])
